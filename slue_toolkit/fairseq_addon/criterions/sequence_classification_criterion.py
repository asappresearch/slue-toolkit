...
# add slue_ as prefix of the registerred name in case there are conflicts in future
@register_criterion(
    "slue_sequence_classification_multilabel", dataclass=SequenceClassificationCriterionConfig
)
class SequenceClassificationMultilabelCriterion(FairseqCriterion):
    def __init__(self, cfg: SequenceClassificationCriterionConfig, task: FairseqTask):
        super().__init__(task)
        self.cfg = cfg
        self.num_classes = len(task.label2id)
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        net_output["pooled"] = torch.sigmoid(net_output["pooled"])
        ...
        # )
        threshold = 0.5
        pred = net_output["pooled"] > threshold
        # pred = net_output["pooled"].argmax(dim=-1)

        correct = (pred == sample["target"]).long().sum()
        sample_size = int(loss.numel()/self.num_classes)
        if reduce:
            loss = loss.sum()
