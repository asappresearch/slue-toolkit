...
class DataSetup():
	...
	def prep_data(self, split_name, label_type="raw", get_map_files=False):
		...
		if not os.path.exists(tag_id_fn):
			...
			write_to_file("\n".join(list(unique_tags)), os.path.join(self.data_dir, f"{label_type}_tag_lst_ordered"))
		else:
			tag2id = load_pkl(tag_id_fn)
		if get_map_files:
			return 1
		# Tokenize data
		encodings = self.tokenizer(texts, is_split_into_words=True, return_offsets_mapping=True, padding=True, truncation=True)
...
def train_module(data_dir, model_dir, train_dataset, val_dataset, label_list, model_type):
	...
	model = DebertaForTokenClassification.from_pretrained(f'microsoft/{model_type}', num_labels=len(label_list))
...
class Eval():
	def __init__(self, data_dir, model_dir, model_type, label_list, eval_label, eval_asr=False):
		"""
		...
		self.model = DebertaForTokenClassification.from_pretrained(best_model_ckpt_dir, output_loading_info=False)
	...
	def get_entities(self, tag_lst, score_type, text_lst=None):
		...
		entity_tag_lst = []
		entity_flag, entity_info, entity_lst, entity_name, prev_tag = False, [], [], None, "O"
		for tag_idx, tag_name in enumerate(tag_lst):
			if tag_name != "O":
				if "B-" in tag_name or ("I-" in tag_name and self.reduce(tag_name)!=self.reduce(prev_tag)): # start of a new entity
					if entity_flag: # record the previous entity first
						self.update_entity_lst(entity_lst, entity_name, score_type, entity_info)
	...
	def get_tag_map(self, indices=False, tag_names=False):
		...
		raw_to_combined_tag_map = load_pkl(os.path.join("slue_toolkit/label_map_files", "raw_to_combined_tags.pkl"))
		if indices:
			...
			id2tag_combined = load_pkl(os.path.join(self.data_dir, "combined_id2tag.pkl"))
			tag2id_combined = load_pkl(os.path.join(self.data_dir, "combined_tag2id.pkl"))
			raw_to_combined_id = {}
	...
	def get_entity_tags(self, predictions, labels, score_type, gt_text=None, gt_tags=None, pred_text=None):
		if "combined" in self.eval_label:
			tag_map_dct = self.get_tag_map(indices=True)
			predictions = [[tag_map_dct[item] for item in prediction] for prediction in predictions]
		...
		entity_predictions_reformat = self.get_entities(entity_predictions[0], score_type, pred_text)
		if self.eval_asr:
		...
		assert len(entity_predictions_reformat[0]) == len(set(entity_predictions_reformat[0]))

		return entity_predictions_reformat, entity_labels_reformat

	def run_inference(self, score_type, eval_dataset_pred, eval_texts_gt, eval_tags_gt=None, eval_texts_pred=None):
		all_labels = []
		all_predictions = []
		if "combined" in self.eval_label:
		...
		for idx, batch in enumerate(data_loader):
			...
			if self.eval_asr:
				...
				entity_predictions, entity_labels = self.get_entity_tags(predictions, labels, score_type, \
					eval_texts_gt[idx], eval_tags_text, eval_texts_pred[idx])
			else:
				entity_predictions, entity_labels = self.get_entity_tags(predictions, labels, score_type)
